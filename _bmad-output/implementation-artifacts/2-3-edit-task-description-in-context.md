...
- [ ] Add in-list edit-mode UX and interaction flow in `TodoList` (AC: 1, 2)
  - [ ] Add per-item edit mode state (single active editor at a time unless existing patterns support multiple safely)
  - [ ] Render inline edit input with semantic controls (`Save`, `Cancel`) using `<button>` elements
  - [ ] Preserve keyboard operability (Enter to save, Escape to cancel)
  - [ ] Keep non-edited rows usable; only the affected item controls should be pending/disabled during save
- [ ] Add frontend update adapter behavior for description updates (AC: 1, 2)
  - [ ] Reuse and extend `frontend/src/features/todos/api/updateTodo.ts` for description payload support (do not create duplicate adapter)
  - [ ] Ensure standardized backend error-envelope message extraction is preserved on non-2xx responses
  - [ ] Add/extend adapter tests in `frontend/src/features/todos/api/updateTodo.test.ts`
- [ ] Add mutation orchestration for description edits with optimistic safety (AC: 1, 2)
  - [ ] Extend or add hook behavior in `frontend/src/features/todos/hooks/` to support optimistic description update and rollback on failure
  - [ ] Preserve per-item pending tracking via `Set<number>` pattern (align with stories 2.1 and 2.2)
  - [ ] Preserve scoped per-item error state and retry affordance
  - [ ] Invalidate/refetch authoritative todos query after settle
- [ ] Ensure backend update contract validates description updates (AC: 1, 2)
  - [ ] Confirm `PATCH /todos/{todo_id}` request schema accepts/validates description updates with existing constraints
  - [ ] Ensure invalid description returns standardized validation error envelope with no persistence change
  - [ ] Ensure not-found behavior remains standardized (`TodoNotFoundError` path)
- [ ] Wire app-level integration and tests (AC: 1, 2)
  - [ ] Wire edit handlers/props in `frontend/src/app/App.tsx`
  - [ ] Add/extend UI tests for inline edit save/cancel/validation/error-retry flows
  - [ ] Add/extend backend tests for successful description patch and invalid payload behavior
...
- Story file created and set to `ready-for-dev` for developer handoff.
- Implementation intentionally not started in this workflow.
